  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const SELLER_API_URL =
        import.meta.env.VITE_SELLER_URL || "http://localhost:3000/api/seller/";

      const response = await fetch(`${SELLER_API_URL}auth/login`, {
        method: "POST",
        headers: {
      ...
      setAuthenticated(username, rememberMe);
